...
class HammerProject : public ProjectExplorer::Project {
   ...
   public:
      HammerProject(ProjectManager* manager,
                    const main_target* mt,
                    bool main_project = false);
      ~HammerProject();
...
class HammerProjectFile : public Core::IDocument {
   ...
   public:
      HammerProjectFile(HammerProject* parent,
                        QString fileName);
      ~HammerProjectFile();
      ...
      ReloadBehavior
      reloadBehavior(ChangeTrigger state,
                     ChangeType type) const override;
